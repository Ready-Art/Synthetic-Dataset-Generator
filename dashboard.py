...
class SmoothCanvas(FigureCanvasTkAgg):
    """Canvas that defers ALL rendering to the Tk event loop.

    On VirGL/Mesa, the default resize() triggers a synchronous OpenGL
    draw call that blocks the UI thread for 30-120 seconds. This override
    skips the synchronous draw entirely and schedules an idle-time redraw.
    """
    _resize_after_id = None  # Class-level debounce token

    def resize(self, event=None):
        # Update internal figure dimensions WITHOUT triggering a draw
        if event and hasattr(event, 'width') and event.width > 1:
            dpi = self.figure.get_dpi()
            self.figure.set_size_inches(event.width / dpi, event.height / dpi)

        # Cancel any previously scheduled redraw (debounce rapid resize events)
        if SmoothCanvas._resize_after_id is not None:
            try:
                self.get_tk_widget().after_cancel(SmoothCanvas._resize_after_id)
            except Exception:
                pass

        # Schedule a single deferred redraw 150ms from now
        # This means dragging the window edge only triggers ONE redraw
        SmoothCanvas._resize_after_id = self.get_tk_widget().after(
            150, self._deferred_draw
        )

    def _deferred_draw(self):
        """Called from the Tk event loop after resize settles."""
        SmoothCanvas._resize_after_id = None
        try:
            if self.get_tk_widget().winfo_exists():
                self.draw_idle()
        except Exception:
            pass  # Widget may have been destroyed between schedule and call
...
def draw_issue_graph(canvas_widget, height=400):
    """Lightweight time-series graph optimized for VirGL/VM environments."""
    canvas_widget.delete("all")

    # Lower DPI + fixed size prevents initial render hangs
    fig = Figure(figsize=(12, 3.5), dpi=80, facecolor='#1e1e24')
        ...
        for rect in bar:
            ...
            if h > 0:
                ax.annotate(f'{int(h)}', xy=(rect.get_x() + rect.get_width()/2, h),
                            ...
                            fontweight='bold', antialiased=False)

    max_val = max(max(c) for c in counts.values()) if any(any(c) for c in counts.values()) else 5
    ax.set_ylim(0, max_val + 2)
    ax.yaxis.set_major_locator(ticker.MaxNLocator(integer=True))

    ax.set_xlabel('Time Window (Last 60 Minutes)', fontsize=11, fontweight='bold', color='#e0e0e0')
    ax.set_ylabel('Issue Count', fontsize=11, fontweight='bold', color='#e0e0e0')
    ax.set_title('Issue Detection Dashboard', fontsize=13, fontweight='bold', color='#ffffff', pad=10)
    ax.set_xticks(x)
    ax.set_xticklabels(x_labels, rotation=0, ha='center', fontsize=9, color='#c0c0c0')
    ...
    ax.legend(loc='upper right', fontsize=9, framealpha=0.85, facecolor='#2a2a35',
